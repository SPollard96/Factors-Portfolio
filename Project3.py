...
#Cash = 1000000
#NbAssets = 50
#df_Uni = df_Universe_EUR_PX.copy()
#df_Uni = df_Uni.loc['2017-12-29 00:00:00':,]
#df_Div = df_EUR_Dividends.copy()
#df_Div = df_Div.loc['2018-12-31 00:00:00':,]
#df_MktCap = df_Float_Market_Cap.copy()
#df_MktCap = df_MktCap.loc['2018-12-31 00:00:00':,]
#df_BP = df_Book_To_PX_ratio.copy()
#df_BP = df_BP.loc['2018-12-31 00:00:00':,]
#df_Return_Com_Eqy = df_Return_Com_Eqy.loc['2018-12-31 00:00:00':,]
#df_Asset_Growth = df_Asset_Growth.loc['2018-12-31 00:00:00':,]
    
    
#df_strat = pd.DataFrame(np.zeros_like(df_Uni),index=df_Uni.index, columns=df_Uni.columns)
#df_strat = df_strat.loc['2018-12-31 00:00:00':,]
#df_perfs = df_Uni.copy()
#df_Shares = df_strat.copy()

#Start the momemtum strategy
#df_perfs = df_perfs.pct_change(N)
#df_perfs = df_perfs.loc['2018-12-31 00:00:00':,]
#
##Rebalance starts at True to build day 1 portfolio
...
#    if RebalancingN != 0 and i != 0 :
#        Rebalance = (i % RebalancingN == 0)
#        Cash =  pd.Series(df_Shares.loc[df_perfs.index[i-1]].multiply(df_Uni.loc[df_perfs.index[i-1]], fill_value = 0)).sum()
#        
#    if Rebalance == True:
#        #get the stocks selected by the strategy
##        Selection = Momentum(df_perfs.iloc[i,:])
#        Selection = ThreeFactors(df_perfs.iloc[i,:], df_MktCap.iloc[i,:], df_BP.iloc[i,:])
#        #set the number of shares for the selected stocks depending of their weights
#        for index, value in Selection.items():
#            df_Shares.at[df_perfs.index[i],index] = ((1/NbAssets) * Cash)/df_Uni.at[df_perfs.index[i],index]
#    else :
#        #if no rebalance copy last allocation
#        df_Shares.iloc[i] = df_Shares.iloc[i-1]
#        #check dividends
#        df_DividendsReceived = pd.Series(df_Shares.loc[df_perfs.index[i]].multiply(df_Div.loc[df_perfs.index[i]], fill_value = 0))
#        #buy assets of the paying stocks with the money received
#        df_DividendsReceived = df_DividendsReceived.div(df_Uni.loc[df_perfs.index[i]], fill_value = 0)
#        df_Shares.iloc[i] = pd.Series(df_Shares.iloc[i]).add(df_DividendsReceived)
#    
#    Rebalance = False
#        
#df_strat = df_Shares.mul(df_Uni.loc['2018-12-31 00:00:00':,], axis='columns')
##    df_strat = df_strat.dropna()
#df_strat["Index"] = df_strat.sum(axis=1)
#df_strat["Index"] = 100*(df_strat["Index"] / df_strat["Index"].iloc[0])
##    df_strat["Index"].plot()
#
#Perf = df_perfs.iloc[90,:]
#Size = df_MktCap.iloc[90,:]
#Book_to_Price = df_BP.iloc[90,:]
#Return_Com_Eqy = df_Return_Com_Eqy.iloc[90,:]
#Asset_Growth = df_Asset_Growth.iloc[90,:]
#
#Perf.sort_values(axis=0, ascending = False, inplace=True)
#r1 = Perf.rank(ascending = False)
#Size.sort_values(axis=0, ascending = True, inplace=True)
